...
│   │   └── vector_store.py                 # Qdrant client + in-memory fallback
...
│       ├── __init__.py
│       ├── main.py                         # FastAPI app with CORS for NextJS
│       └── routes/
│           ├── __init__.py
...
- **TODO:** Deploy Qdrant Docker container for production use
...
### Phase 5: Web Interface 🔧 PARTIALLY IMPLEMENTED

- **Backend:** FastAPI with 3 API routes implemented
    - `POST /api/generate-data` → Run Phase 1 pipeline
    - `GET /api/search-vector` → Query similar tasks
    - `POST /api/discover-process` → GNN inference → Petri net JSON
- **Frontend:** NextJS scaffold in `fe/` (default template)
- **TODO:**
    - Build custom UI pages for data visualization
    - Integrate graph visualization library (React Flow / D3.js)
    - Display Petri net results and cost predictions
...
2. **Build NextJS frontend** with process visualization components
3. **Deploy Qdrant** via Docker for persistent vector storage
4. **Evaluate** model with conformance metrics (fitness, precision, simplicity)
5. **Integration testing** of full pipeline (data → GNN → Petri net → UI)
